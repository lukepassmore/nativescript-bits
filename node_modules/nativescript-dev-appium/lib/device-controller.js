...
class DeviceManger {
    ...
    startDevice(args) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (process.env["DEVICE_TOKEN"]) {
                ...
                const allDevices = yield mobile_devices_controller_1.DeviceController.getDevices({ platform: device.platform });
                const foundDevice = mobile_devices_controller_1.DeviceController.filter(allDevices, { token: device.token.replace("emulator-", "") })[0];
    ...
    uninstallApp(args) {
        return __awaiter(this, void 0, void 0, function* () {
            if (args.isIOS) {
                yield mobile_devices_controller_1.IOSController.uninstallApp(args.device, args.appPath, args.appiumCaps["bundleId"]);
            }
            else {
                yield Promise.resolve(mobile_devices_controller_1.AndroidController.uninstallApp(args.device, args.appiumCaps["appPackage"]));
            }
        });
    }
    static kill(device) {
    ...
    static getDefaultDevice(args) {
        let device = new mobile_devices_controller_1.Device(args.appiumCaps.deviceName, args.appiumCaps.platformVersion, undefined, args.appiumCaps.platformName.toLowerCase(), undefined, undefined);
        device.config = { "density": args.appiumCaps.density, "offsetPixels": args.appiumCaps.offsetPixels };
        return device;
    }
    getPackageId(device, appPath) {
        const appActivity = (device.type === mobile_devices_controller_1.DeviceType.EMULATOR || device.platform === mobile_devices_controller_1.Platform.ANDROID) ? mobile_devices_controller_1.AndroidController.getPackageId(appPath) : mobile_devices_controller_1.IOSController.getIOSPackageId(device.type, appPath);
        return appActivity;
    }
}

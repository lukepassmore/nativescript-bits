...
class IOSController {
    ...
    static startSimulator(simulator, directory = os_1.tmpdir()) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let responce = yield utils_1.waitForOutput(process, /Instruments Trace Complete:/ig, /Failed to load/ig, IOSController.DEVICE_BOOT_TIME);
            if (responce === true) {
    ...
    static installApp(device, fullAppName) {
        return __awaiter(this, void 0, void 0, function* () {
            if (device.type === enums_1.DeviceType.DEVICE) {
                ...
                const result = utils_1.executeCommand(`${IOSController.SIMCTL} install ${device.token} ${fullAppName}`);
                console.dir(result);
            }
        });
    }
    ...
    static uninstallApp(device, fullAppName, bundleId = undefined) {
        return __awaiter(this, void 0, void 0, function* () {
            bundleId = bundleId || IOSController.getIOSPackageId(device.type, fullAppName);
            if (device.type === enums_1.DeviceType.DEVICE) {
                try {
                    yield IOSController.stopApplication(device, bundleId);
                }
                catch (error) {
                    console.dir(error);
                }
                utils_1.wait(1000);
                const result = utils_1.executeCommand(`ideviceinstaller -u ${device.token} -U ${bundleId}`);
            }
            else {
                utils_1.executeCommand(`${IOSController.SIMCTL} ${device.token} terminate ${bundleId}`);
                utils_1.wait(1000);
                const result = utils_1.executeCommand(`${IOSController.SIMCTL} uninstall ${device.token} ${bundleId}`);
            }
        });
    }
    static reinstallApplication(device, fullAppName, bundleId = undefined) {
        return __awaiter(this, void 0, void 0, function* () {
            bundleId = bundleId || IOSController.getIOSPackageId(device.type, fullAppName);
            yield IOSController.uninstallApp(device, fullAppName, bundleId);
            yield IOSController.installApp(device, fullAppName);
        });
    }
    ...
    static startApplication(device, fullAppName, bundleId = undefined) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (device.type === enums_1.DeviceType.DEVICE) {
                let startProcess;
                try {
                    startProcess = yield (yield IOSController.getDl()).start([{ "ddi": undefined, "appId": bundleId, "deviceId": device.token }])[0];
                ...
                const result = utils_1.executeCommand(`${IOSController.SIMCTL} launch ${device.token} ${bundleId}`);
                console.dir(result);
                Promise.resolve(result);
            }
        });
    }
    static startSimulatorProcess(udid, cwd = os_1.tmpdir()) {
    ...
    static startRecordingVideo(device, dir, fileName) {
        ...
        if (device.type === enums_1.DeviceType.DEVICE) {
            ...
            videoRecoringProcess = child_process_1.spawn(`xcrun`, ['simctl ', 'io', device.token, 'recordVideo', `'${pathToVideo}'`], {
                cwd: process.cwd(),
                shell: true,
                stdio: 'inherit'
            });
    ...
    static getIOSPackageId(deviceType, fullAppName) {
        let result = "";
...
IOSController.WAIT_DEVICE_TO_RESPONCE = 180000;

"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const frame_comparer_1 = require("./lib/frame-comparer");
var frame_comparer_2 = require("./lib/frame-comparer");
exports.FrameComparer = frame_comparer_2.FrameComparer;
function createFrameComparer() {
    return new frame_comparer_1.FrameComparer();
}
exports.createFrameComparer = createFrameComparer;
// const storage = resolve("./");
// const tempFolder = `${storage}/temp`;
// const fullVideoName = "https://media.w3.org/2010/05/sintel/trailer.mp4";
//const videoComparer: VideoComparer = new VideoComparer(fullVideoName);
// async function test() {
//     await videoComparer.processVideo(tempFolder);
//     let result = await videoComparer.compareImageFromVideo(resolve(storage, "test55.png"), 50, 60);
//     return result;
// }
// test();
//# sourceMappingURL=index.js.map